...
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): after a witnessed park the REAL `src/locationPrivacy.ts` stays pinned through his 10:12–10:14 fixes (and a 29 km/h-every-second worst case), re-arms a real drive-away only after 15 s and 150 m (`src/parkRearm.ts`), restarts the run at a red light or a gap, a reconnect is instant, and without a witnessed park every output is identical to the pre-fix module (N = the pre-fix module re-arming on the 26 km/h fix).
