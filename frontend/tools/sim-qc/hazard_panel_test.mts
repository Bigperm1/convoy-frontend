...
// D · the phone (Jeff, 2026-09-24: "WHERE IS THE HAZARDS BUTTON ON THE PHONE?"): the same tiles, one source of truth
{
  ...
  ok("D6 the panel wears the weather forecast card's floor + tinted GlassFill and is no longer a Modal sheet (Jeff, 2026-09-24: same opacity as the weather panel)", sheet.includes('backgroundColor: "rgba(24,24,28,0.66)"') && sheet.includes("<GlassFill tintColor={hudTint()}") && !sheet.includes("<Modal") && sheet.includes('style={[StyleSheet.absoluteFill, { zIndex: Z_CARD - 1 }]} onPress={() => close("tap")}'));
  ...
  ok("D9 NO BackHandler in the sheet (2026-09-25: a hardwareBackPress listener left the panel open-but-unpainted on the iOS sim, 4-of-4; auto-close covers Android Back)", !sheet.includes("BackHandler.addEventListener") && !/import \{[^}]*BackHandler/.test(sheet) && sheet.includes("NO BackHandler here"));
