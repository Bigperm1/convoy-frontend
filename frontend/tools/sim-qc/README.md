...
- `node --experimental-strip-types tools/sim-qc/car_feed_leak_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "it should not follow me when i discconect from car play… fix it and lock it"): the car GPS watch has ONE creator (`src/carFeedOwner.ts`) — two concurrent starts make one native watch, a stop mid-start / with zero deliveries / over 5 connect cycles leaves 0, a fix after the release never reaches the car store; the REAL `navNotification.ts` driven through carPlayBootstrap's connect/disconnect (G; G0 = the pre-fix file leaking one watch); static checks on the release paths and map.tsx's cancellation-safe phone watcher. CARPLAY.md §6c.
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): after a witnessed park the REAL `src/locationPrivacy.ts` stays pinned through his 10:12–10:14 fixes (and a 29 km/h-every-second worst case), re-arms a real drive-away only after 15 s and 150 m (`src/parkRearm.ts`), restarts the run at a red light or a gap, a reconnect is instant, the witness drops the drive's leftover latch (W: 12 and 26 km/h fixes 40 s after the disconnect share the car spot) and hydrate never restores a latch over a witnessed spot (H2), and without a witnessed park every output is identical to the pre-fix module (N / W0 / H2a = the pre-fix module sharing live).
