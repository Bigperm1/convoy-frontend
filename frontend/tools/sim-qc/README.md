...
- `node --experimental-strip-types tools/sim-qc/car_feed_leak_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "it should not follow me when i discconect from car play… fix it and lock it"): the car GPS watch has ONE creator (`src/carFeedOwner.ts`) — two concurrent starts make one native watch, a stop mid-start / with zero deliveries / over 5 connect cycles leaves 0, a fix after the release never reaches the car store; the REAL `navNotification.ts` through connect/disconnect (G; G0 = the pre-fix file leaking one watch, G6 = a failed background start); the REAL `drawTelemetry.ts` car-row drop (T); the REAL `carPlayBootstrap.ts` connect/disconnect (CP); the REAL `AndroidAutoRoot.tsx` through three car sessions in one process (AA; AA0 = round 3's root); map.tsx's phone-watcher effect extracted and run (M); static release-path and inventory checks (F); pinned owner values (P1). CARPLAY.md §6c.
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): the REAL `src/locationPrivacy.ts` after a witnessed park stays pinned through his 09-25 and 08-29 walks (recorded, densified, 41 s stuck on the multipath point, 14–31 s of 16 km/h readings first), two-outlier and ping-pong attacks and a car-park loop + outlier, and re-arms a real drive-away under `src/parkRearm.ts` v3 (pull-away ≤ 35 s, highway ≤ 30 s, red light ≤ 20 s after it, stop-sign grids ≤ 60 s, stop-and-go ≤ 75 s, jams on the slow path ≤ 5–9 min); a late storage read cannot be raced (Y); every witness starts the proof over; the witness drops the drive's latch (W/H2/S2); the head-unit sources (K); pinned rule values (V1); without a witnessed park every output is identical to the pre-fix module (U1). the review's canyon walker model (Z). Negative controls: N / W0 / H2a / K0 on the pre-fix module, Y0 / X0 / X0b / Z0 / Q0 on round 3 (732c8a6e).
