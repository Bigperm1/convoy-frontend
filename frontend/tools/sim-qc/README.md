...
- `node --experimental-strip-types tools/sim-qc/nav_lock_test.mts` — 🔒 **THE NAVIGATION LOCK** (Jeff, 2026-09-17: "i dont want
  ...
  (`{/* … */}` form between JSX children; 221 regions since 2026-09-17)
...
- `node --experimental-strip-types tools/sim-qc/nav_lock_regions.mts --check|--apply plan.json` / `--verify` — places the 🔒 region
  markers and PROVES each is a comment: the file is parsed by the TypeScript compiler AND @babel/parser (Metro's) with and
  without them and printed with comments removed — the two programs must be identical (inside JSX children or a template literal a `//` line is TEXT; between a
...
- `node --experimental-strip-types tools/sim-qc/car_zoom_step_test.mts` and `…/headunit_cam_delay_test.mts` — the HEAD-UNIT camera (CarPlay / Android Auto, 2026-09-25): the PRODUCTION CarMapView + SelfCarModel camera code lifted by `headunit_cam_harness.mts` and run against a modelled native camera (iOS `setCamera` 'none' does not cancel an animation; Android's does). `car_zoom_step` = the +/- ease, the hold, the Crew return, the one-owner writer gate, the tails; `headunit_cam_delay` = the same camera with every native flyTo/easeTo starting 0–600 ms late and ending up to 80 ms late — the closed loop (`src/camRepair.ts`) must bring the parked camera to the owner's framing within 1.5 s of an animation's REAL end, never pump when idle, never fight a pinch, never judge a late camera report against a newer write, restore the WRITTEN pose (not the target), and stay time-bounded on a map that never agrees (3 tries + one giveup per episode, then slow retries; ≤ 6 unconfirmed writes and ≤ 12 rows a minute). Round 10 (`LR` / `RN`): no try on the zoom hold's lapse tick; a driver who taps again right after a repair is never rationed. Round 11 (the loop NEVER flies — one instant push of the full written pose; round 10's landing hold removed): `FL1–FL8` PIN the documented residual (a native fly that starts 150–600 ms late is cut by its own landing push — CARPLAY.md; a change to any pinned number fails), `RH` the written heading restored (not the car's), `RP` no freeze / hop on a moving car, `RZ` no fly chain, `RS` / `RR` every try resolves (`op=ok|giveup|superseded`; a try superseded before its verdict is dropped, bounded by the 30-a-minute ceiling), `RL` the `cam-lat` latency crumb (start / end / lag per owner fly, ease and sampled instant write; ≤ 20 rows a session, ≥ 3 s apart; `RL3` the echo rule — a moving car's ease is not dated by the lockstep's last push). Round 11 review: `SN1–SN3` PIN the loop's restore cut in its primary case (Codex's stranded Crew overview: one frame of 1.80 / 45° / 90° — the same cut the recenter itself makes when its write is not lost, SN2); `car_zoom_step` CL18 gates the echo rule directly. `car_zoom_step` CL12–CL17 gate the same rules in `src/camRepair.ts` directly.
...
- `node --experimental-strip-types tools/sim-qc/crew_widget_test.mts` — the iOS 27 full-page widget's crew ring row (`src/crewWidgetRule.ts`): driving/live/parked order and nearest-first inside each bucket, the 5-slot cap, the LIVE pill count, the republish throttle (no WidgetKit wake on an unchanged row), and the snapshot gate (never while navigating, never backgrounded).
...
- `node --experimental-strip-types tools/sim-qc/garage_cars_test.mts` — the Showroom Garage's logic (`src/garageCars.ts`, `src/garageStore.ts`, `src/carScan.ts` delivery/reconcile, `src/components/showroom/tier.ts`) with the native and network edges stubbed (`tools/sim-qc/garage/`): which cars each tier owns and which view it sees, what "Drive this today" writes for every car kind (one `updateSettings` write; a ready scan is parked for a non-scan car and restored for a scan), scan delivery and the reconcile guards, account switches, prices from `src/pricing.ts` only. Built 2026-09-22 with the new Garage.
- `node --experimental-strip-types tools/sim-qc/weather_word_test.mts` — the weather word and glyph (`src/weatherLayer.ts`): the sun stays in the chip until the deck is overcast (OpenWeather 801–803 → partly, 804 → cloudy; Jeff's 09-22 "cloudy when sunny" was 77 % thin cirrus, METAR BKN250), a forecast block is "rain" only at pop ≥ 0.5 AND ≥ 0.5 mm (the measured `id=500 pop=0.23 rain3h=0.16` block is not raining), and an arrival inside 90 min reads the destination's CURRENT conditions instead of the 3-hour block (Scout's "raining at 18 degrees" on a sunny 3-minute drive).
- `node --experimental-strip-types tools/sim-qc/car_feed_leak_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "it should not follow me when i discconect from car play… fix it and lock it"): the car GPS watch has ONE creator (`src/carFeedOwner.ts`) — two concurrent starts make one native watch, a stop mid-start / with zero deliveries / over 5 connect cycles leaves 0, a fix after the release never reaches the car store; the REAL `navNotification.ts` through connect/disconnect (G; G0 = the pre-fix file leaking one watch, G6 = a failed background start); the REAL `drawTelemetry.ts` car-row drop (T); the REAL `carPlayBootstrap.ts` connect/disconnect (CP); the REAL `AndroidAutoRoot.tsx` driven by the native hold / ctx / disconnect receipts — three car sessions in one process, a JS reload that joins a live session, a session destroyed before JS started, the init receipt before or after the mount, a binary with no receipts (AA; AA0 = round 3's root, AA6-0 / AA7-0 = round 4's); map.tsx's phone-watcher effect extracted and run, including the no-car self-stop after a lost Android Auto disconnect (M; M6-0 = 90dd1484); static release-path and inventory checks (F); the settle clock is monotonic (S1); pinned owner values (P1). CARPLAY.md §6c.
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): the REAL `src/locationPrivacy.ts` after a witnessed park stays pinned through his 09-25 and 08-29 walks (recorded, densified, 41 s stuck on the multipath point, 14–31 s of 16 km/h readings first), two-outlier and ping-pong attacks and a car-park loop + outlier, and re-arms a real drive-away under `src/parkRearm.ts` v4 (300 m floor: pull-away ≤ 40 s, highway ≤ 30 s, red light ≤ 25 s after it, stop-sign grids ≤ 65 s, stop-and-go ≤ 91 s); slow congestion stays pinned 30 min and goes live ≤ 60 s after the jam clears (Q); the fifth review's walkers at 8 / 12 m noise, 1 Hz and Lite cadence, walk-then-sit and a moving-walkway concourse stay pinned (WK); the real-Δt claim, the 10 s baseline and the run check each alone (V5–V7); a late storage read cannot be raced (Y), and a FAILED one (rejects once / 3× / an unparsable record / a hung read) is retried, never counted as done — the walk is never shared live and the saved record stays intact, an Android Auto TTL lapse with failing reads never shares the walk, a caller of a hung read is released by the retry or within 10 s, a latch whose last driving fix is over 90 s old (or in the future: a clock moved back) shares nothing and renews nothing, the head-unit TTL and the re-arm proof survive clock jumps, and a property test of random device-clock jumps after a witnessed or a lost disconnect never shares a walk or writes a spot while a real drive stays live (HF, HF9g, PT; the witnessed park survives a 25 h-old or clock-skewed record and a process death mid-CarPlay — HF10, HF10a-0 / HF10c-0 = 36c17f1e; a CarPlay session that delivered no fix still leaves a witness (its own key, `src/parkWitness.ts`) and a restart keeps Jeff's walk pinned, a failing read of that key is retried fail-closed, the key clears after the proof or a reconnect and a restart after either is not re-pinned — HF13, HF13b–e, HF13-0 / HF13e-0 = 4ef3b2d9; the privacy clock does not drift under dense calls and logs rollbacks — CLK1 / CLK2, CLK1-0 = 36c17f1e; the paused-monotonic residual is pinned — KF1; HF0 = 0255f36a, HF7-0 / HF8-0 = 47db1aca, HF9-0 / HF9b-0 = 7da32366, HF9c-0 / HF9e-0 / HF9f-0 = 90dd1484, HF9g-0 / PT-0 = b286ad9b; V8); every witness starts the proof over; the witness drops the drive's latch (W/H2/S2); the head-unit sources (K); pinned rule values (V1); without a witnessed park every output is identical to the pre-fix module (U1). the review's canyon walker model (Z). Negative controls: N / W0 / H2a / K0 on the pre-fix module, Y0 / X0 / X0b / Z0 on round 3 (732c8a6e), Q0 / WK0 on round 4 (8a6fa0aa), V5c / V6c / V7c on this rule with the real-Δt claim, the 10 s baseline or the run check undone, WK6-0 on round 4.
- `node --experimental-strip-types tools/sim-qc/depart_facing_test.mts` — a GPS course noted below the car spot's own moving line (`SPOT_WRITE_MAX_SPEED_MS` 1.5 m/s) is the phone's direction, not the car's (`src/departureBearing.ts` `noteCourse(heading, speedMs)`): Olaf's 2026-09-22 14:14Z plot on a 0.28 m/s course 295 now answers the spot's 110; drive-stop-plot inside 90 s still answers the course; the witnessed-park spot still wins after expiry (OTA-AZ guard); a 2.0 m/s U-turn park is still the facing (C8 — the case a 2.5 m/s gate got 180° wrong).
- `node --experimental-strip-types tools/sim-qc/frame_pacer_test.mts` — the nav draws at 60 fps on every surface (`src/framePacer.ts`; Jeff, 2026-09-23: "make sure the nav is stuck at 60fps for all 4 platforms"). Synthetic vsync clocks: a 60 Hz panel (iPhone, CarPlay) is never paced, even with 12 ms-late callbacks; 75/90/120/144 Hz panels (Android phones, Android Auto heads, ProMotion) average 60 drawn frames a second with no two closer than 10 ms; an idle restart re-anchors without a catch-up burst; `navMapFps()` re-sends the Mapbox cap on Android right after the first render (rnmapbox 10.3.1 drops a `preferredFramesPerSecond` set before its MapView is created — not after map load, which waits for tiles); and E runs the pacer through ConvoyMapbox's own pump guard (constants read from the source) with runaway 0.5–3 ms rAF callbacks — the old flat 16 ms timer fallback settled at 42.1 fps on 1 ms callbacks (Codex review of 39cdbc34), slot-timed it holds 60. Field proof: Android `heat-probe` rows' `cam=` and `tick=` at ≤ ~3600 a minute (they reached ~4500 before).
- `node --experimental-strip-types tools/sim-qc/speed_limit_cover_test.mts` — where the posted-limit cache is complete (`src/speedLimitCover.ts`): a landed Overpass fetch covers the snap inside `FETCH_RADIUS_M − SNAP_TOLERANCE_M` = 1470 m of the point it was centred on (Overpass `around:` is segment distance, measured 2026-09-22), and beyond that `src/speedLimit.ts` shows no sign rather than whichever long way pokes into the old circle — Olaf's 2026-09-22 14:31:39 phantom `80>50 cls=tertiary` tier-2 ding on Highway 17, replayed on live OSM geometry with the snap alone still saying 50; the 1470/1471 boundary; a 30 zone at 1450 m still shows; the honest drops (a 70 ramp at 116, a 30 off-ramp at 56, the ferry lane's service 30 at 64) still resolve; and B1 pins the KNOWN-OPEN untagged-ramp case (Olaf 14:41:59 beside Burns Drive) that this fix does NOT touch.
- `node --experimental-strip-types tools/sim-qc/self_lift_rule_test.mts` — the self car's lift (`src/selfLiftRule.ts`): 0 on a road, the off-road lift in a lot / driveway / a footprint's roof + 2 m (Jeff, 2026-09-10: "the road rule is pronounced for the car at a light and slow speeds"). A red light with a blank and a bad second never lifts; speed ≥ 18 km/h, the route within 25 m, or a rendered road within 25 m each ground it at once; off-road evidence must hold 2.5 s; no evidence never starts a lift; the Streets v8 classifier (a parking aisle / driveway is NOT a road, an alley is); the ease lands exactly.
```
