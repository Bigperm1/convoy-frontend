...
**⛔ SUPERSEDED 2026-09-06 by `HANDOFF-2026-09-06.md` — this file is now the 09-02 → 09-06 fix log (history; read it for root causes, not for state).** ~~**This file is CURRENT STATE.**~~ It supersedes `HANDOFF.md` (whose header is stale at
...
```
`tools/ota/ow.key` is regenerated on demand and DELETED immediately after use with <!-- doc-check:ignore: the key file is transient by design — it holds a secret and this repo is public, so it must never persist on disk -->
...
- 📣 **Crew note SENT 22:20 PDT** (Claude, WhatsApp desktop → "Hairpin App Testing", on Jeff's "send it"): OTA-P…T in
  plain terms — event invites (tag your club even on public, Going / Not going, Send the invite, 24h/2h follow-ups with
  the forecast, Post a meet on the card), exit zoom hold, lot-entrance corner release, bigger phone car, Show-map cover,
  compass toggle — plus that Codex (ChatGPT) now reviews my work as a second set of eyes and caught a real bug before
  anyone drove on it. Pickup = the red pill. Asked for: car leaving the line on a highway, a wide corner, a yo-yo exit, a
  missing event push.
- 📣 **Olaf iMessaged 22:29 PDT** (Claude, from Jeff's Messages; "Delivered Quietly" — his notifications are silenced, Notify
  Anyway NOT pressed): why his HUD looks bigger than Jeff's. **VERIFIED by measuring the two screenshots** (same iPhone,
  both 1290×2796): chip FRAMES identical (weather chip edges 36→286 px on both), status clock identical (40 px both), but
  every app text ~1.3× larger on his (speedo digit 1.37×, search placeholder 1.30×, crew pill 1.29×) = iOS Dynamic Type
  (Text Size) a couple of notches up; the app never caps `allowFontScaling`. Sent him the per-app fix (Settings →
  Accessibility → Per-App Settings → Hairpin → Larger Text → Off / slider to default) and the whole-phone path, and
  Jeff's ask to double-check and report the setting. **Open design choice for Jeff:** cap the map HUD (weather, speedo,
  crew pill, search bar, tab labels) at ~1.15× via `maxFontSizeMultiplier` so phones match, leaving lists/settings scaling.
...
## 11 · 2026-09-06 — Codex security review → backend release (Claude reviewed + released)

Jeff had Codex hunt bugs/vulnerabilities and handed over its HANDOFF; Codex was not allowed to deploy. Everything below
was VERIFIED against the real checkouts, not the handoff text.

**Backend `~/convoy-backend` commit `466f2a3` (master)** — six fixes by Codex, one addition by Claude:
- Social sign-in: only a verified email claim inside the signed Apple/Google token can link an existing account (the app's
  separately submitted email is ignored); a different provider subject never replaces a linked one (409).
- Clubs: invite code only to current admins; private rosters only to members; admin/co-admin rights require membership.
  Pre-check through the API: all 3 clubs have admin + co-admins inside `members` — nobody gets locked out.
- Live PTT: `floor_acquire` checks channel membership (error frame otherwise); a deleted user's token cannot open a socket.
- Scan credits: authoritative Supabase `scan_slots` synced into Mongo before every count/retry; released slots never count or
  retry; an unreadable store is a 503, never a slot issued from stale state (`select_rows_strict`: service role, exact counts, paginated).
- `/scan/mine`: by account ownership + server-issued slot ids, never the editable handle; store errors are 503, not an empty success.
- Claude: `/health` now returns `rev` (RENDER_GIT_COMMIT prefix) so a deploy is verified live, not inferred from timing.
- `tests/`: Codex's 47-test offline suite ported (`conftest`, `test_scan_slots` (replaced — the old fixture had no strict-read
  mock and failed 16/19 on the new code), `test_scan_store`, `test_security_regressions`); 47 passed from the repo path with
  Codex's venv (`~/Documents/Codex/2026-09-06/h/work/test-venv`). `tests/test_convoy_v3.py` is the old LIVE suite — do not run blindly.
- **Push to master (= Render deploy) was blocked by the auto-mode classifier** → Jeff pushes: `git -C ~/convoy-backend push origin master`.

**Frontend `cae7bbb`** (pushed): `supabase/migrations/20260906000100_account_scan_recovery.sql` + `tools/sim-qc/nav_start_permissions_test.cjs` <!-- doc-check:ignore: renamed to nav_start_permissions_test.mts 2026-09-25 -->
(6/6: iOS+Android × undetermined/denied/granted start nav with NO prompt). The `src/navNotification.ts` edit of unknown provenance
that shipped in OTA-AB was Codex's — mystery closed. **No new OTA.**

**Supabase migration — apply AFTER the backend is live** (handoff order): backfill from trusted slots = 0 rows today (only slot row is
`qa-20260902-081500`; the mirror was a no-op until the service key landed 09-03), then revoke anon/authenticated/public EXECUTE on
`scan_jobs_for_handle(text)` (pre-state: postgres, anon, authenticated, service_role all EXECUTE). Then the ownership backfill from the
AUTHENTICATED `users.car_scan_id` pointer (PUT /auth/profile, echoed): `jeff-20260902-193844` → Jeff, `enablewhore-20260901-210315` → Olaf.
The other six ownerless `car_scan_jobs` rows (Jeff 08-29, Olaf 09-01-185736 + 09-03 failed, three probes) stay ownerless — superseded, no
trusted record, never inferred from a handle. Baseline before deploy: `/scan/mine` as Jeff = `jeff-20260902-193844` + `jeff-20260829-141551`
(handle RPC); after = the pointer scan only.

**⚠ Jeff's iPhone 15 Pro Max runs Codex's local Xcode DEBUG build since 10:38 PDT** (`launch_kind=updates-disabled`, `update_id` NULL,
channel "") — it receives NO OTA until build 75 is reinstalled from TestFlight. **⚠ MongoDB Atlas MCP:** disabled for the whole Atlas org
("MCP access is disabled…") — an Org Owner must enable AI client access; the backend has no local `.env`, so Mongo facts come through the
backend API with the sim's session token.

**RELEASED 2026-09-06 12:03 PDT (VERIFIED live, not inferred):** Jeff pushed → Render deployed `466f2a3` (`/api/health` → `rev=466f2a3`).
Migration `20260906000100_account_scan_recovery` applied through the Supabase MCP (listed as version `20260906190359`); grants on
`scan_jobs_for_handle(text)` are now `postgres, service_role` only, and the shipped anon key gets `42501 permission denied for function`
on a live call (Jeff + Enablewhore). Ownership rows set: 1 + 1 (Jeff, Olaf). `/scan/mine` as Jeff on the new backend = `jeff-20260902-193844`
only (the superseded 08-29 scan no longer listed — by design); invite code hidden on the two clubs Jeff does not admin, visible on YVRGRC;
public rosters still visible. Not exercised live: Olaf's recovery (no token), social login, PTT floor, credit sync under a failing store —
those rest on the 47-test suite. Jeff reinstalled TestFlight 75 at 11:57 and was on OTA-AB (`01a0778e`) four seconds later. Atlas
"Allow users to connect AI clients" turned ON (read-only) 12:00 PDT — this session's Mongo connector still reports disabled (HYPOTHESIS:
cached at connect; a new session settles it).
