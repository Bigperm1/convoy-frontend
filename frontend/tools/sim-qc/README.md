...
- `node --experimental-strip-types tools/sim-qc/car_feed_leak_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "it should not follow me when i discconect from car play… fix it and lock it"): the car GPS watch has ONE creator (`src/carFeedOwner.ts`) — two concurrent starts make one native watch, a stop mid-start / with zero deliveries / over 5 connect cycles leaves 0, a fix after the release never reaches the car store; the REAL `navNotification.ts` through connect/disconnect (G; G0 = the pre-fix file leaking one watch, G6 = a failed background start); the REAL `drawTelemetry.ts` car-row drop (T); the REAL `carPlayBootstrap.ts` connect/disconnect (CP); map.tsx's phone-watcher effect extracted and run (M); static release-path and inventory checks (F); pinned owner values (P1). CARPLAY.md §6c.
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): the REAL `src/locationPrivacy.ts` after a witnessed park stays pinned through his 09-25 and 08-29 walks (recorded, densified, a 41 s multipath stick, a car-park loop + one outlier), re-arms a real drive-away under `src/parkRearm.ts` v2 (pull-away ≤ 30 s, highway ≤ 20 s, a red light does not restart it, stop-sign grids ≤ 60 s, stop-and-go ≤ 75 s), every witness starts the proof over, the witness drops the drive's latch (W/H2/S2), the head-unit sources (K: an iOS cold CarPlay disconnect is witnessed, a cold connect clears yesterday's witness, the map mirror cannot create or cancel one), pinned rule values (V1), and without a witnessed park every output is identical to the pre-fix module (U1; N / W0 / H2a / K0 = the pre-fix module sharing live).
