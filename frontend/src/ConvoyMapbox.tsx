...
// 1.9 → 1.6 (2026-07-11 drive feedback) → 1.25 (Jeff, 8/19 drive home: "shrink the
// size a bit cause it is a little too big and touches the turnbyturn banner"). The
// arrow sits at the camera's focal point while the banner occupies the top of the
// screen, so an oversized arrow reaches it at city zoom, where the model scale
// curve is at its largest relative to the road. OTA-tunable.
...
function ConvoyMapbox(props: ConvoyMapboxProps) {
  ...
  const routeProj = useMemo(() => {
    if (!navigationActive || !selfCar) {
      ...
      // cannot inherit a stale arc position (CarMapView already did this).
  ...
  // distance gate. OTA-tunable.
  const SNAP_HDG_GRACE_MS = 4000;
  const _distSnap = navigationActive && routeProj != null && routeProj.distM <= SELF_SNAP_M;
  // ⚠ RAW COURSE, NOT THE CAMERA HEADING (Jeff, 8/19: "when turning the markers do
  // not really face the direction of the route/road").
  // This gate used to compare the route's bearing against camHeadingRef — the
  // LOW-PASSED CAMERA heading, which by design lags the car through a corner. So
  // in every turn the measured disagreement swung past 45°, the snap released
  // exactly mid-corner, and the marker fell back to raw GPS position AND to that
  // same lagging camera heading — i.e. the one moment the driver is watching the
  // nose, it stopped following the road. The gate exists to catch a WRONG turn
  // onto a parallel street, and the driver's actual course is the honest input for
  // that; it is only consulted above SELF_SNAP_MOVING_MS, where a GPS course is
  // trustworthy. Camera heading remains the fallback when there is no course.
  ...
  if (_distSnap && _travelHdg != null && (userSpeedMs ?? 0) >= SELF_SNAP_MOVING_MS) {
    ...
    // manoeuvre the driver is watching. A wrong turn, by contrast, keeps
    // disagreeing (and pulls the car past the 60 m distance gate, which still
    // releases immediately and is not softened here). So: start a timer when the
    // angle first disagrees, and only release once it has held for
    // SNAP_HDG_GRACE_MS. Clears the moment the angle agrees again.
    const nowMs = Date.now();
    // dt is clamped so a backgrounded/throttled render gap cannot dump seconds
    // into the accumulator in a single frame.
