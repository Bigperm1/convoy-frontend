...
// Visual constants â keep PTT cleanly centered. Stage = ripple/tick area; button sits in middle.
...
export default function ComsScreen() {
  ...
  // Persist active community to settings â map.tsx reads this to scope the presence channel.
  ...
  // â it sometimes returns 0 or a stale cached value AFTER `stopAndUnloadAsync`,
  ...
  // Adaptive audio quality â pulls the latest proximity tier from the shared
  ...
  // shows who's *live* on the map â the member list shows the full crew.
  useEffect(() => {
  ...
  // /(app)/_layout.tsx â so PTT plays on every tab (map, music, hub, etc).
  ...
  const startRec = async () => {
    ...
    // Comms Live OFF â radio silence, refuse to broadcast.
    ...
    try {
      // Switch to RECORDING audio category â this enables `.playAndRecord`
      ...
      // Capture our own start timestamp â expo-av's status.durationMillis is
  ...
  const stopRec = async () => {
    ...
    try {
      ...
      // Floor at 200ms â anything shorter is almost certainly an accidental
  ...
  return (
    <SafeAreaView style={styles.c} edges={["top"]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 110 }} showsVerticalScrollIndicator={false}>
        ...
        {/* ===== PTT button â Apple liquid-glass design, perfectly centered ===== */}
        <View style={styles.pttSection}>
          {/* Adaptive audio quality badge â dot color + label scale with how
          ...
          <View style={styles.tierBadge}>
            ...
            <Text style={styles.tierLabel}>
              ...
              {proximityPeers > 0 ? ` Â· ${proximityPeers}` : ""}
            </Text>
          </View>
          <View style={styles.stage}>
            ...
            {/* Layer 3: tick dial â minimal, evenly spaced, very subtle */}
          ...
          <Text style={styles.hint}>
            {busy ? "Sendingâ¦" : recording ? "Release to broadcast" : `Press & hold to broadcast to ${activeCommunity?.name || "channel"}`}
          </Text>
        </View>

        {/* ===== Channels (Hub-style cards) =====
            Communities with `walkie_enabled === false` are hidden â admins use that
          ...
          <Glass radius={16} style={{ marginHorizontal: 18, marginTop: 6 }}>
            <View style={{ padding: 16, alignItems: "center" }}>
              ...
              <Text style={styles.emptyText}>
                None of your communities have Walkie-Talkie Connect enabled. An admin can turn it on from Hub â community â settings.
              </Text>
            </View>
          </Glass>
        )}
        ...
        {active && members.length > 0 && (
          <>
            <Text style={styles.section}>{activeCommunity?.name || "Crew"} Â· {members.length} member{members.length === 1 ? "" : "s"}</Text>
                ...
                <Glass key={m.id} radius={16} style={{ marginBottom: 8, marginHorizontal: 18 }}>
                  <TouchableOpacity testID={`play-${m.id}`} style={styles.msg} onPress={() => playMessage(m)}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.msgMeta}>{Math.round((m.duration_ms || 0) / 1000)}s Â· {new Date(m.created_at).toLocaleTimeString()}</Text>
                    </View>
...
// Minimal Apple-style tick dial â 24 evenly spaced ticks. Centered via parent stage.
...
const styles = StyleSheet.create({
  ...
  // computed from peer distances. Purely informational â no tap target.
  tierBadge: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
  ...
  // Crew roster â slim member row that fits inside the Glass card. Avatar +
