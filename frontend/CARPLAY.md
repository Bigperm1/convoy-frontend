...
| zoom − / zoom + | CPBarButton (top bar; AA map strip) | `zoomStep` ∓0.5 → `applyZoomEased` (2026-09-25, Jeff: "the carplay zoomout seems like its capped at a distance and is not smooth" → "It creeps back in"). Each press **eases in 0.28 s** (`CAR_ZOOM_STEP_MS`, `src/carZoomStep.ts`; re-aimable, velocity-continuous, never a cut; pitch and heading are not touched). The framing is **absolute for the 15 s hold** (`CAR_ZOOM_HOLD_MS`): speed no longer pulls it — nor does the corner/roundabout zoom-in, which is suspended for the hold. Press limit 4 levels from the speed zoom (`CAR_USER_ZOOM_BIAS_LIMIT`), floor 10.5. Then it **eases home in 1.2 s** (`CAR_ZOOM_RELEASE_MS`) to the live speed zoom, parked or moving; the release's target follows a changing speed zoom at ≤ 0.5 level/s (a step advance or nav start mid-release is not a cut). A press **during the Crew return fly re-aims the fly** (one flyTo from where the camera is to chase ± the step, over the fly's remaining time — an iOS `setCamera` 'none' cannot stop a fly); a press while the overview or a fly moves the map starts from the **visible** zoom. Tap-zoom (two-finger tap out / double tap in, 1 level) takes the same path; pinch stays instant (1:1) and takes over from the zoom being applied, mid-ease or mid-release. **Any gesture while a native animation or a Crew overview owns the camera** (a return fly, a re-aimed fly, crewFit's easeTo, the overview itself) — pinch, recenter, compass, the AppState re-assert — goes through the camera owner (`takeOverNativeCam`: the fly is re-aimed onto the gesture's framing, or the overview ends and the fly home takes the camera back), never an instant write that loses to the fly on iOS or strands its landing on Android; a pinch, like the buttons, ends a Crew overview. **One camera owner, enforced:** every head-unit camera write goes through SelfCarModel.pushCam or the owner's two instant writers (`applyZoomNow`, `ownerSetPose` — gestures and system corrections alike: the layout correction, the cold-start snap at first paint); the only other write is crewFit's overview easeTo, which reconciles itself. A system correction never ends a Crew overview and never overrides a +/- ease; mid-fly it re-aims the fly. An instant write **records what it wrote** and the lockstep re-seeds from that however late its next push comes (a bare one-shot used to snap a stopped car to whatever the targets had become — a +/- press snapped the pitch 45°); `ownerSetPose` honours the north-up latch. A return fly owns the camera for `RETURN_FLY_GRACE_MS` (100 ms) past its JS deadline, because the native fly ends a frame or so later (HYPOTHESIS: the dispatch latency is under that). So does crewFit's 600 ms easeTo into the overview: a way home that starts inside it — through its native end + that grace, not its JS deadline — goes by a short fly (Codex's final pass: a nearby crew, recenter 600 ms after Crew → the snap lost to the easeTo's last frame on iOS, parked at z 15 / pitch 0 / heading 0 with nothing owed). And a write that still lands inside any native animation's tail is **re-applied once after it** (`reapplyAfterRef`, owed by `carCamJob`), never dropped. Gate TL: every gesture −20…+120 ms around each animation's end (crewFit's easeTo, a fly, a re-aimed fly, the 7 s return), nearby and wide crew, parked, iOS and Android, the owner's framing held for 20 s. Those graces are a latency ASSUMPTION (a native animation that starts 150 ms late still overwrote a write made after them — Codex, round 7), so the head unit also runs a **closed loop** (`src/camRepair.ts`): it keeps what it last wrote (SelfCarModel.pushCam publishes every push; the instant writers record theirs) and what the map REPORTS (`onCameraChanged` / `onMapIdle`: zoom, pitch, heading), and once nothing owns the camera, the map has been still for 150 ms and it reports the camera AFTER the last write (its native `timestamp` — a report captured before the write and delivered late is never judged; a write that drew no report is judged after 1 s), a disagreement beyond 0.05 zoom / 1° owes ONE corrective write that **restores what was written** — never the speed target: ONE instant push seeded with the full written pose — zoom, pitch AND heading (the north-up override still wins) — and **never a fly** (round 11: the round 9/10 280 ms repair fly, used when the gap exceeded 0.2 zoom / 2°, flew to the car's heading rather than the written one — a parked map turned ~20° — and every owner intent inside it re-aimed it, 460–2000 flyTo / 10 min). The price: a restore is a one-frame cut of exactly the lost write's size (harness FL1/FL2: 4.80 levels for a foreign write to z 12). **DOCUMENTED RESIDUAL — the loop's primary case** (round 11 review, fight [medium]): Codex's stranded Crew overview (nearby crew, recenter 720 ms after Crew, iOS, native start 150 ms late, parked) is restored by ONE instant push — **z 1.80 / pitch 45° / heading 90° in one frame, 214 ms after the recenter** (e492b0e1 flew it with no cut); Codex's shape ×8 two seconds apart is six such cuts in 10 s. It adds no new size: the same recenter whose write is NOT lost (native start 0 ms) is the owner's own one-frame 1.80 / 45° / 90° cut at +30 ms, no loop row — the exit from a near overview (z ≥ 14) is a snap by design (`crewReturnEdge` flies only below `CHASE_ZOOM`); the restore makes it 184 ms late. Android has no loss here (its 'none' write cancels the easeTo: the owner's own 1.785 / 44.6° / 89.2° cut). Only an animated repair (ruled out: the loop never flies) or an animated near-overview exit would remove it. Pinned by gate SN1–SN3 — a change to any number fails. Bounds (round 9): every owner intent — a gesture, a system correction, a Crew press — opens an episode; an agreement confirmed after a try clears its count; 3 consecutive unconfirmed tries (backing off 150 → 450 → 1350 ms) then ONE `cam-repair op=giveup` row for the episode; after that, slow retries 20 s apart and doubling (≤ 5 min) so a camera left wrong comes back with nothing happening; per minute at most 6 corrective writes the map never confirmed, 30 of any kind, and 12 rows (the rest counted into the next row's `drop=`). Round 11 rationing: a try an owner intent supersedes before it was judged is DROPPED (never counted as unconfirmed) and resolves in one `op=superseded counted=0` row (`counted=1` when it had already been judged wrong — it stays counted); only a try judged against a post-write report or the 1 s no-report fallback counts toward the 6 a minute, so a map that stops reporting while the driver keeps tapping is bounded by the 30-a-minute ceiling (harness RR1: 30 a minute, 12 rows a minute + `drop=`). Every try resolves: ok, giveup or superseded (gate RS). Receipt `cam-repair surf=car op=push\|ok\|giveup\|superseded n= dz= dp= dh= rep= ep= [counted=] [drop=]` — `rep` = the judged report's capture time − the write's (≥ 0 for a real lost write), `ep` = the episode's age. A moving car never involves it (it pushes every frame). Gate `headunit_cam_delay_test`: native start delay 0–600 ms + end jitter, every gesture × animation, the owner's framing within 1.5 s of an animation's REAL end, 60 s idle with zero writes, no fight with a pinch; round 9 adds reports delivered 0–400 ms late (zero loop rows), repeated lost writes (no strand), a map deaf for 8 s (home by the slow retry), the time caps, and the repair restoring the written pose; round 11 adds RH (the written heading restored, not the car's), RP (no freeze or hop on a moving car), RZ (no fly chain), RS / RR (every try resolves; the bound when every try is superseded) and RL (the latency crumb). Round 10's **landing hold** (`camFlyHold`: past its JS deadline + grace a fly kept the camera while the map showed it not yet started or still moving) is **removed in round 11** — it judged "still moving" by the JS receipt time, so late camera reports froze a moving car's camera (up to 1.08 s) and then hopped it (+13.6 m), and a no-op corrective fly held its 1 s cap (an 18 m one-frame hop on pulling away). Owner flies are owned until their JS deadline + the 100 ms grace again, as at e492b0e1. **DOCUMENTED RESIDUAL — a native fly that starts late is cut by its own landing push** (harness, parked, native start delay 150 / 300 / 600 ms; HYPOTHESIS for a head unit — the real delay is unmeasured, which is what `cam-lat` is for): Android Crew return 0.03 / 0.29 / 1.46 levels (pitch 0.2 / 1.8 / 9.0°, heading 0.4 / 3.6 / 18°); Android return re-aimed by a '+' 0.21 / 1.96 / 4.35 levels (heading 1.8 / 19.5 / 47.5°); Android short fly inside crewFit's easeTo 0.04 at 150 ms, none at 300 / 600; iOS return re-aimed 4.24 levels (heading 46°) at 600 ms only; iOS Crew return and iOS short fly: none up to 600 ms. Every case still ends on the flown pose. Pinned by gate FL1–FL8 — a change to any number fails it. Kept from round 10: a 15 s zoom hold that has lapsed and not yet been released stands the loop down (a try on the lapse tick used to fly to the old framing, then cut 1.4–1.6 levels; gate LR — with round 11 a loss caught before the lapse is the one instant restore, and the release eases from it). **`cam-lat` (round 11 — measure, don't model):** per car session ≤ 20 rows, ≥ 3 s apart — `cam-lat surf=car kind=fly\|ease\|push ms= start= end= lag= n= [echo=]` for each owner fly (the Crew return, a re-aimed fly), crewFit's easeTo and a sample of instant writes (≤ 5 a session, ≥ 60 s apart): `start` = ms from the JS call to the first native camera report captured after it, `end` = to the next onMapIdle after that (`-` when the map never idled — a moving car), `lag` = the largest JS receipt − native capture over its reports, `n` = its reports, `ms` = the requested duration. **Echo rule** (round 11 review, platform [medium]): until the start is dated, a report showing EXACTLY a pose an instant write made in the 250 ms before the call (`CAM_LAT_ECHO_MS`) is that write's own report delivered late, not the call's start — skipped and counted in `echo=` (printed only when > 0; a sampled push's own pose is never one). Without it a MOVING car's crewFit easeTo, called a frame after the lockstep's last push, read that push's report: start = 1–17 ms at every native delay (gate RL3; parked rows unchanged — RL1). Where a push preceded the call, `start` is the first frame that MOVED (one vsync after the animation's first tick, in the harness). A late fly cut on Android reads end − start < ms. No location in the row. Gate `car_zoom_step_test` S20–S24 scans the whole file, follows the camera ref (helpers, imports, aliases, bracket access, spreads) and plants each shape as a negative control. (There is no style-load seed: it could never run and was removed; the pending re-centre is unreachable today — CarMapView mounts only with a fix and the fix is never lost within a mount.) **A fly's landing never outlives it:** the parked pump lands it the moment its time is up (it used to wait for the car to move and then beat any framing set since — a 1.0-level cut), and any leftover is retired before an instant gesture writes. One camera writer: SelfCarModel.pushCam (its parked pump drives a stopped car). Receipt `car-zoom op=step\|release … via=push\|park n=` (≤ 1 row / 2 s). Gate `tools/sim-qc/car_zoom_step_test.mts` — runs the production CarMapView/SelfCarModel code (`headunit_cam_harness.mts`) |
| crew | CPMapButton | `crewFit` — frames self + all peers, north-up, 7 s camera hold (`CREW_RETURN_MS`, same clock as the phone; Jeff 2026-09-25), then the 1.8 s fly home — **moving or stopped** (2026-09-25: the parked pump runs the way home on the first tick after the deadline; it used to wait for the next GPS fix, 09-20 +23.75 s). Zoom, recenter and compass end it at once — inside crewFit's own 600 ms easeTo (`CREW_FIT_EASE_MS`) by a 0.28 s fly, since a 'none' push cannot cancel that easeTo on iOS. Receipt `crew-home surf=car why=deadline\|ended late= fly=1\|short\|0 via=` |
