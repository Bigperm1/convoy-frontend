...
type Tick = {
  ...
  /** The fast path's own course test; defaults to headingOff (in these traces the heading IS the course). */
  courseOff?: boolean;
...
[[5, 7, true, 30, null], [7, 6, true, 22, null], [8, 6, true, 400, 15], [6, 6.5, true, 400, 9], [6, 7, true, 400, 14], [4, 8, false, 392, 22]].forEach(([d, v, off, dm, db], i) =>
...
// ── W: A STOP WITH A TRANSIENT GPS OFFSET, NO COURSE (Codex pass 2, reproduced) ───────
// Sticky heading aligned (legacy headingOff=false, six-tick threshold), no course (fast path inert),
// three fixes 86–90 m off then back to 4 m, speed 0. The legacy path must hold as it did before
// 09-11; merging the fast path's course test into headingOff dropped the threshold to three and tripped.
const stopTicks: Tick[] = [];
for (let i = 0; i < 30; i++) stopTicks.push({ pos: 10 * i, d: i % 2 ? 8 : 4, speedMs: 10, headingOff: false, headingKnown: true, courseOff: false, dManeuverM: 500 });
