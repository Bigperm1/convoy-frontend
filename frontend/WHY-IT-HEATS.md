...
2. **The thermal-throttle detector.** `expo-battery` is already a dependency. *(Update 2026-08-30: this said "already wired in `src/powerMode.ts`" — that module was DELETED on 2026-08-29 when Battery Saver was removed <!-- doc-check:ignore: names a deliberately-deleted file -->, because eco gave zero measured protection against the runaway. `src/heatProbe.ts` is now expo-battery's only consumer, which is why the package must not be dropped as cleanup.)* Sample `getBatteryLevelAsync()` + `getBatteryStateAsync()` every 60 s during nav, alongside `carConnected`, AppState, and the build/update id. **A drive where level is flat or falling while state is `CHARGING` is iOS throttling the charger — the exact symptom you saw, captured automatically, on every drive, from every tester, with no instruction.** That converts "it got hot that one time" into a per-build number you can compare.
