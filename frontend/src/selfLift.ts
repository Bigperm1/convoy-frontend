...
type SurfaceEv = { at: number; speedMs: number | null; roadHit: boolean | null; buildingH: number | null; lot: boolean };
...
export function reportSelfLiftEvidence(surface: LiftSurface, ev: { speedMs: number | null; roadHit: boolean | null; buildingH: number | null; lot?: boolean }): LiftState {
  const now = Date.now();
  _ev[surface] = { at: now, speedMs: ev.speedMs, roadHit: ev.roadHit, buildingH: ev.buildingH, lot: !!ev.lot };
  const m = merged(now);
  const next = liftDecide(_state, m, now, _offRoadLiftM);
  if (next.targetM !== _state.targetM && _rows < ROWS_MAX) {
    _rows += 1;
    try {
      logEvent(`self-lift surf=${surface} to=${next.targetM.toFixed(1)} from=${_state.targetM.toFixed(1)} why=${next.why} spd=${m.speedMs == null ? "?" : (m.speedMs * 3.6).toFixed(0)} nav=${m.navDistM == null ? "-" : m.navDistM.toFixed(0)} road=${m.roadHit == null ? "?" : m.roadHit ? 1 : 0} bld=${m.buildingH == null ? "-" : m.buildingH.toFixed(0)}${ev.lot ? " lot=1" : ""}`);
    } catch {}
  }
  _state = next;
  return next;
...
export function logSelfLiftQuery(surface: LiftSurface, q: { roads: number; drivable: number; drivableM: number | null; propertyM: number | null; buildings: number; ms?: number; nearestCls?: string | null }, ev: RoadEvidence): void {
  ...
  _qRows += 1;
  try {
    logEvent(`self-lift q surf=${surface} roads=${q.roads} drv=${q.drivable} drvM=${q.drivableM == null ? "-" : q.drivableM.toFixed(0)} propM=${q.propertyM == null ? "-" : q.propertyM.toFixed(0)} bldN=${q.buildings} ms=${q.ms ?? "?"} road=${ev.roadHit == null ? "?" : ev.roadHit ? 1 : 0} bld=${ev.buildingH == null ? "-" : ev.buildingH}${ev.lot ? " lot=1" : ""} cls=${q.nearestCls ?? "-"}`);
  } catch {}
}
