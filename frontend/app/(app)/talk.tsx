...
// Visual constants Ã¢ÂÂ keep PTT cleanly centered. Stage = ripple/tick area; button sits in middle.
...
export default function ComsScreen() {
  ...
  // Persist active community to settings Ã¢ÂÂ map.tsx reads this to scope the presence channel.
  ...
  // Ã¢ÂÂ it sometimes returns 0 or a stale cached value AFTER `stopAndUnloadAsync`,
  ...
  // Adaptive audio quality Ã¢ÂÂ pulls the latest proximity tier from the shared
  ...
  // shows who's *live* on the map Ã¢ÂÂ the member list shows the full crew.
  useEffect(() => {
  ...
  // /(app)/_layout.tsx Ã¢ÂÂ so PTT plays on every tab (map, music, hub, etc).
  ...
  const startRec = async () => {
    ...
    // Comms Live OFF Ã¢ÂÂ radio silence, refuse to broadcast.
    ...
    try {
      // Switch to RECORDING audio category Ã¢ÂÂ this enables `.playAndRecord`
      ...
      // Capture our own start timestamp Ã¢ÂÂ expo-av's status.durationMillis is
  ...
  const stopRec = async () => {
    ...
    try {
      ...
      // Floor at 200ms Ã¢ÂÂ anything shorter is almost certainly an accidental
  ...
  return (
    <SafeAreaView style={styles.c} edges={["top"]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 110 }} showsVerticalScrollIndicator={false}>
        ...
        {/* ===== PTT button Ã¢ÂÂ Apple liquid-glass design, perfectly centered ===== */}
        <View style={styles.pttSection}>
          {/* Adaptive audio quality badge Ã¢ÂÂ dot color + label scale with how
          ...
          <View style={styles.tierBadge}>
            ...
            <Text style={styles.tierLabel}>
              ...
              {proximityPeers > 0 ? ` ÃÂ· ${proximityPeers}` : ""}
            </Text>
          </View>
          <View style={styles.stage}>
            ...
            {/* Layer 3: tick dial Ã¢ÂÂ minimal, evenly spaced, very subtle */}
          ...
          <Text style={styles.hint}>
            {busy ? "SendingÃ¢ÂÂ¦" : recording ? "Release to broadcast" : `Press & hold to broadcast to ${activeCommunity?.name || "channel"}`}
          </Text>
        </View>

        {/* ===== Channels (Hub-style cards) =====
            Communities with `walkie_enabled === false` are hidden Ã¢ÂÂ admins use that
          ...
          <Glass radius={16} style={{ marginHorizontal: 18, marginTop: 6 }}>
            <View style={{ padding: 16, alignItems: "center" }}>
              ...
              <Text style={styles.emptyText}>
                None of your communities have Walkie-Talkie Connect enabled. An admin can turn it on from Hub Ã¢ÂÂ community Ã¢ÂÂ settings.
              </Text>
            </View>
          </Glass>
        )}
        ...
        {active && members.length > 0 && (
          <>
            <Text style={styles.section}>{activeCommunity?.name || "Crew"} ÃÂ· {members.length} member{members.length === 1 ? "" : "s"}</Text>
                ...
                <Glass key={m.id} radius={16} style={{ marginBottom: 8, marginHorizontal: 18 }}>
                  <TouchableOpacity testID={`play-${m.id}`} style={styles.msg} onPress={() => playMessage(m)}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.msgMeta}>{Math.round((m.duration_ms || 0) / 1000)}s ÃÂ· {new Date(m.created_at).toLocaleTimeString()}</Text>
                    </View>
...
// Minimal Apple-style tick dial Ã¢ÂÂ 24 evenly spaced ticks. Centered via parent stage.
...
const styles = StyleSheet.create({
  ...
  // computed from peer distances. Purely informational Ã¢ÂÂ no tap target.
  tierBadge: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
  ...
  // Crew roster Ã¢ÂÂ slim member row that fits inside the Glass card. Avatar +
