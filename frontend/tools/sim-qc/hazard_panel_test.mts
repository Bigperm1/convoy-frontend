...
// G · round three (Jeff, 2026-09-25): compass tile, category-coloured pins, the card, the pill, the Scout-timed prompt
{
  ...
  { const car = readFileSync(new URL("../../src/carplay/CarMapView.tsx", import.meta.url), "utf8");
    ...
    ok("G12 the head-unit Crew hold is the phone's 7 s CREW_RETURN_MS (Jeff, 2026-09-25: \"on carplay the crew button does not have the zoom in timer\")", region.length > 0 && /^import \{[^}]*\bCREW_RETURN_MS\b[^}]*\} from '\.\.\/crewReturn';$/m.test(car) && region.includes("camHoldUntilRef.current = Date.now() + CREW_RETURN_MS;") && !/Date\.now\(\) \+ 15000/.test(region)); }
}
