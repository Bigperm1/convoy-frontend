...
export default function MapScreen() {
  ...
  // Multi-route state Ã¢ÂÂ primary "Route Line" (blue) + alternates (gray)
  ...
  // Search bar visibility Ã¢ÂÂ auto-hides when navigation starts so the destination
  ...
  // Layers control state Ã¢ÂÂ driven by the new bottom-right Layers FAB.
  ...
  // Position history buffer Ã¢ÂÂ keeps the last 30s of GPS samples so the user
  ...
  // map stops chasing the user Ã¢ÂÂ they're free to inspect any region. Tapping
  ...
  // Music broadcast toast Ã¢ÂÂ surfaced when the community admin pushes a track
  // via Music screen Ã¢ÂÂ "Ã°ÂÂÂµ jeff: Smooth Operator Ã¢ÂÂ Sade". Auto-dismisses 5s.
  ...
  // Hail toast Ã¢ÂÂ surfaced when a peer pushes us via POST /api/notifications/hail.
  // Two delivery paths feed this:
  //   - OS push notification while app is foregrounded (via hailBus from _layout)
  //   - Raw WebSocket frame (via livePtt listener Ã¢ÂÂ see useEffect below)
  ...
  // Right-edge Navigation Action Drawer Ã¢ÂÂ peeked 80% off-screen by default
  ...
  // Preview-card collapse state Ã¢ÂÂ when the driver starts moving (or taps the
  ...
  // When nav stops, we don't auto-show Ã¢ÂÂ the driver explicitly taps the FAB
  // or returns to preview. This mirrors Apple/Google Maps behavior.
  useEffect(() => {
    if (navMode === "turn-by-turn") setSearchVisible(false);
  }, [navMode]);

  // Auto-collapse the preview card once the driver starts moving (speed Ã¢ÂÂ¥ 5 km/h).
  ...
  // Optional Convoy alert sound Ã¢ÂÂ chime when a NEW community hazard appears
  ...
  useEffect(() => {
    ...
    if (settings.alertSound && prevHazardIdsRef.current.size > 0) {
      ...
      if (newOnes.length > 0) {
        // Soft platform chime Ã¢ÂÂ best-effort, silent if unavailable
        try {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Color-rank: green (fastest) Ã¢ÂÂ orange (mid) Ã¢ÂÂ red (slowest). Cast to
  ...
  // Turn-by-turn engine Ã¢ÂÂ speaks instructions, advances steps, computes ETA / distance remaining
  ...
  // Delete a hazard (by id) Ã¢ÂÂ used by the long-press / right-click flow on
  ...
  // it Ã¢ÂÂ destructive operations always require a deliberate second tap.
  ...
  // `components/StepDrawer` Ã¢ÂÂ we just hold a ref so we can drive open/close.
  ...
  // Tap a route polyline on the map Ã¢ÂÂ keep ONLY that route, fire up nav with
  // chase-cam (45ÃÂ° behind the car), and pop the step drawer for 3s so the
  ...
  const handleSelectRoute = (index: number) => {
    ...
    setShowSteps(false);
    setNavMode("turn-by-turn");
    // Force chase cam on Ã¢ÂÂ `mapView` controls bearing-lock + 45ÃÂ° pitch downstream.
  ...
  // ===== Hail subscription =====
  //
  // Peers can hail us via two transports Ã¢ÂÂ both feed `hailBus`:
  ...
  useEffect(() => {
    const off = hailBus.on(({ fromHandle }) => {
      setHailToast(`Ã°ÂÂÂ¨ ${fromHandle} is hailing you!`);
  ...
  // for a car-enthusiast app Ã¢ÂÂ this is the same cadence Google Maps uses.
  // Also drives a throttled Google Roads speed-limit lookup (1ÃÂ/10s) so the
  ...
  // `settings.speedUnitManual` Ã¢ÂÂ set true when they tap a unit button).
      ...
      try {
        ...
        sub = await Location.watchPositionAsync(
          ...
          (pos) => {
            ...
            // `useEffect([sessionMaxSpeed, ...])` block below Ã¢ÂÂ no duplicate post here.
            if (kmh >= 1) {
              setSessionMaxSpeed((m) => (kmh > m ? kmh : m));
            }
            // Google Roads speed-limit lookup Ã¢ÂÂ throttled to 1/10s so we
            ...
            //     into Washington flips KM/H Ã¢ÂÂ MPH within ~1 minute.
            //   * US Ã¢ÂÂ MPH, everywhere else (Canada/Mexico/EU/etc) Ã¢ÂÂ KM/H.
  ...
  useEffect(() => {
    ...
    // 30-second polling fallback Ã¢ÂÂ catches any hazards missed during a
    ...
    const channel = supabase
      ...
      // DELETE fan-out Ã¢ÂÂ when one driver disputes a hazard ("Not there") the
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        // Global hazard fan-out Ã¢ÂÂ every Convoy user receives every hazard
        ...
        // Music broadcast from the community admin Ã¢ÂÂ surface a non-intrusive
        // toast at the bottom-center "Ã°ÂÂÂµ jeff: Smooth Operator Ã¢ÂÂ Sade ÃÂ· HQ"
        ...
        if (m.type === "music_broadcast") {
          if (m.action === "play" && m.track) {
            ...
            const qLabel =
              m.quality === "lossless" ? " ÃÂ· Ã°ÂÂÂ§ Lossless" :
              m.quality === "high"     ? " ÃÂ· HQ" :
              m.quality === "normal"   ? "" : ""; // hide for standard / unknown
            setMusicToast(`Ã°ÂÂÂµ ${who}: ${m.track.name}${m.track.artist ? ` Ã¢ÂÂ ${m.track.artist}` : ""}${qLabel}`);
  ...
  // Force a full state refresh Ã¢ÂÂ used by the Ã¢ÂÂ³ button in the header. Mirrors
  ...
  const reportHazard = async (kind: string, opts?: { fromVoice?: boolean }) => {
    if (!coords) return;
    // Place pin slightly ahead of the driver's heading (Ã¢ÂÂ40m forward) for accuracy.
  ...
  // Confirm = "still there" Ã¢ÂÂ +1 confirms
  ...
  // Dispute = "not there anymore" Ã¢ÂÂ DELETE the hazard outright.
  ...
  //   2. Supabase DELETE Ã¢ÂÂ Realtime DELETE event fans out to every other
  ...
  // confusing Ã¢ÂÂ drivers tapped Not there and the marker stayed on the map.
  const disputeHazard = async (h: Hazard) => {
    // Snapshot id so we don't double-fetch after `selected` clears below.
    const id = h.id;
    // 1) Optimistic local strip Ã¢ÂÂ tapper sees the marker vanish instantly.
    ...
    try {
      if (SUPABASE_ENABLED && supabase) {
        ...
        if (error) {
          // RLS or column issue Ã¢ÂÂ fall through to backend
      ...
      // Worst case: a stale row reappears on the next refetch Ã¢ÂÂ acceptable
  ...
  // Community-scoped only Ã¢ÂÂ YVRGRC members see other YVRGRC members, period.
  ...
  // Ã¢ÂÂ when off the user vanishes from every peer's map and the map shows no
  // own marker either.
  ...
  const presence = useConvoyPresence(
    ...
    user ? {
      ...
      // Personal best Ã¢ÂÂ live max-of(sessionMaxSpeed, persisted) so peers see
  ...
  useEffect(() => {
    ...
    // First load Ã¢ÂÂ seed seen ids without toasting
  ...
  if (!coords) {
    return <View style={styles.loader}><Text style={{ color: COLORS.textDim }}>LocatingÃ¢ÂÂ¦</Text></View>;
  }
  ...
  return (
    <View style={styles.c}>
      <ConvoyMap
        ...
        // Map view mode (radio choice from Settings Ã¢ÂÂ MAP VIEW). Drives the
        // chase-cam tilt + bearing. Defaults to "heading_up" so nav feels like
        // Waze/Google out of the box.
        mapView={settings.mapView}
        // Layer controls Ã¢ÂÂ driven by the bottom-right Layers FAB.
        mapType={mapType}
        ...
        // ConvoyMap fires `onUserPan` and we flip off follow mode Ã¢ÂÂ the map
        ...
        // by-turn nav is actively running. Pitch defaults to 45ÃÂ° in ConvoyMap.
        navigationActive={navMode === "turn-by-turn" && tbt.active}
        userSpeedMs={coords?.speed}
        // Tap on empty map Ã¢ÂÂ close any open search overlay so the driver can
        ...
        onPeerPress={(p) => {
          // Find the matching presence record (has online_at, etc.) Ã¢ÂÂ fallback to bare peer
        ...
        onExternalAlertPress={(a) => Alert.alert(`${a.type}${a.subtype ? " ÃÂ· " + a.subtype : ""}`, "Live alert from Convoy feed.")}
        onRoute={setRoute}
      />

      {/* ===== Minimal top bar Ã¢ÂÂ Google Maps style =====
      ...
      <View style={styles.topBar} pointerEvents="box-none">
        {searchVisible && (Platform.OS === "web" ? (
          <View pointerEvents="box-none">
            {/* Subtle live pill overlay Ã¢ÂÂ small green dot + live count.
            ...
            {(() => {
              ...
              return (
                <View style={styles.liveOverlay} pointerEvents="none">
                  ...
                  <Text style={styles.liveOverlayText}>{liveCount} live ÃÂ· {visibleHazards.length} alerts</Text>
                </View>
              );
            })()}
            <DestinationSearch
      ...
      {/* ===== Community Routes Ã¢ÂÂ horizontal chip strip (visible when there are shared cruises) ===== */}
      ...
      {/* ===== Route preview Ã¢ÂÂ collapses into Trip Summary pill once moving ===== */}
            ...
            <Glass radius={16} style={styles.tripSummary}>
              <View style={styles.tripSummaryRow}>
                ...
                <Text style={styles.tripSummaryEta} numberOfLines={1}>
                  {(activeRoute?.duration_in_traffic_text || route.duration_text)} ÃÂ· {route.distance_text}
                </Text>
      ...
      {/* ===== Route preview card (full) Ã¢ÂÂ shown when NOT collapsed AND NOT navigating ===== */}
      {destination && route && navMode === "preview" && !previewCollapsed && (
        <Glass radius={20} style={styles.routeCard}>
          <View style={styles.routeRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.routeMeta}>{route.duration_text} ÃÂ· {route.distance_text}{routes[selectedRouteIndex]?.summary ? ` ÃÂ· via ${routes[selectedRouteIndex].summary}` : ""}</Text>
            </View>
          ...
          {/* Alternates picker Ã¢ÂÂ tappable chips when there are >1 routes.
          ...
          <View style={styles.actionRow}>
            ...
            {isAdminOfActive && settings.activeCommunityId && activeMapEnabled && (
              ...
              >
                ...
                <Text style={[styles.secBtnText, { color: COLORS.warning }]}>{savingRoute ? "SharingÃ¢ÂÂ¦" : "Share"}</Text>
              </TouchableOpacity>
            )}
      ...
      {/* ===== Turn-by-turn overlays Ã¢ÂÂ right-edge nav drawer only =====
      ...
      {navMode === "turn-by-turn" && activeRoute && tbt.active && (() => {
        ...
        return (
          <>
            {/* Right-edge Navigation Action Drawer Ã¢ÂÂ TOP of the right stack.
                Peeked 80% off-screen by default; tap the visible 20% edge to
                expand and see maneuver + End. Tap End Ã¢ÂÂ ends nav (which also
            ...
            <NavigationPanel
              ...
              instruction={`${verb}${upcoming?.html ? " ÃÂ· " + upcoming.html : ""}`}
              muted={navMuted}
      ...
      {/* ===== Speedometer HUD (bottom-left glass overlay) =====
          Pulls live speed from coords.speed (m/s) Ã¢ÂÂ km/h. Floors small values
      ...
      {/* Weather HUD Ã¢ÂÂ only shown when weather layer is on and we have data */}
      ...
      {/* ===== Bottom-right floating cluster Ã¢ÂÂ Layers + Directions =====
      ...
      <View pointerEvents="box-none" style={styles.fabStack}>
        {/* Police report button Ã¢ÂÂ top of stack. One-tap: posts a hazard with
        ...
        {/* Road-hazard report button Ã¢ÂÂ same flow with kind='road'. */}
        <TouchableOpacity
        ...
        {/* ===== Recenter FAB =====
            ...
            their position). Tap Ã¢ÂÂ flips `isFollowing` back to true which both
        ...
        {/* Stop Navigation Ã¢ÂÂ appears LEFT of the Directions FAB when a trip
            is active. Identical 42ÃÂ42 footprint, red bg, white X. Tap to
      ...
      {/* ===== Layers + Settings bottom sheet =====
          ...
          rows removed per spec Ã¢ÂÂ those toggles live in the full Settings
                ...
                >
                  <View style={{ flex: 1 }}>
                    <Text style={styles.layerRowLabel}>
                      {mode === 'heading_up' ? 'Ã°ÂÂ§Â­ Heading Up (Chase Cam)' : 'Ã¢Â¬ÂÃ¯Â¸Â North Up (Classic)'}
                    </Text>
                    <Text style={styles.layerRowSub}>
                      {mode === 'heading_up' ? '45ÃÂ° behind your car, rotates with you' : 'Top-down, fixed north orientation'}
                    </Text>
                  </View>
      ...
      {/* Music broadcast toast Ã¢ÂÂ shows up when the convoy admin pushes a
      ...
      {/* Peer hail toast Ã¢ÂÂ bright red, pinned highest so it never gets buried
          under the music broadcast pill. Fed by `hailBus` (push + WS). */}
      <HailToast message={hailToast} />

      {/* ===== Step Drawer Ã¢ÂÂ slide-up turn list =====
...
// AlertItem Ã¢ÂÂ single row inside the Alerts bottom sheet. Icon chip + title +
...
// Plain JS Haversine Ã¢ÂÂ meters between two lat/lng. Used by the Alerts sheet
...
// LayerRow Ã¢ÂÂ a single switch row in the Layers bottom sheet. Colored icon
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    right: 0,
    zIndex: 100,
    // Explicit safe-area paddingTop (replaces SafeAreaView's auto inset) Ã¢ÂÂ
  ...
  // Header row container Ã¢ÂÂ lays out the Glass card and the Search/X square
  ...
  // Live status pill Ã¢ÂÂ same color logic as the old one, just smaller paddings
  ...
  // Refresh + Settings Ã¢ÂÂ a row of small icon buttons. 28ÃÂ28 so the touch
  ...
  // Status footer Ã¢ÂÂ single-line band along the bottom edge of the header card.
  ...
  // doesn't span the screen Ã¢ÂÂ center stays clear for the chase view.
  tripDataRight: {
    position: "absolute",
    right: 12,
    bottom: 100,
  ...
  // End-nav red pill Ã¢ÂÂ bottom-LEFT just above the speedometer. Small footprint
  ...
  // Square Search/X button Ã¢ÂÂ sits to the right of the Map bar in a single
  ...
  // Trip Summary pill Ã¢ÂÂ collapsed view of the route preview card. Renders at
  ...
  // Speedometer HUD Ã¢ÂÂ bottom-LEFT corner above the "Map" footer tab icon.
  // Placed flush against the left edge so the chase-cam center is fully open.
  // ===== Speedometer HUD (bottom-left) =====
  // Square 64ÃÂ64 badge mirroring the right-side FAB stack Ã¢ÂÂ same edge gutter
  ...
  // ===== Bottom-right floating FAB stack Ã¢ÂÂ Alerts (top) + Layers + Directions (bottom).
  // Consistent 48ÃÂ48 rounded squares with semi-transparent dark fills and
  ...
  // Directions = primary action Ã¢ÂÂ Convoy blue. Same 42ÃÂ42 footprint as the
  // others (was 44ÃÂ44 in spec, but matching the rest reads cleaner).
  fabPrimary: { backgroundColor: "rgba(10,132,255,0.92)" },
  // Stop Navigation Ã¢ÂÂ red 42ÃÂ42 button shown LEFT of Directions while a trip
  ...
  // (replaces the old dark header). Green dot + "X live ÃÂ· Y alerts" in a
  // glassy rounded chip Ã¢ÂÂ subtle, glanceable, never blocks the map.
  liveOverlay: {
