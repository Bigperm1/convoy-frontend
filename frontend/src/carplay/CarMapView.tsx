...
import { camObserve, camWrote, camRepairStep, camRepairEpisode, camRepairRow, camLatCall, camLatReport, camLatTick, newCamRepair, type CamObs, type CamPose, type CamRepair } from '../camRepair';
...
export default function CarMapView({ onGLError, attempt = 0, surfaceW = 0, surfaceH = 0 }: Props) {
  ...
  // THE CLOSED LOOP (src/camRepair.ts, 2026-09-25): what the map REPORTS (onCameraChanged / onMapIdle), what the owner last
  // WROTE (SelfCarModel.pushCam publishes every push via camPoseOutRef; applyZoomNow / ownerSetPose record theirs), and
  ...
  const zoomHoldRelease = (now: number) => {
    ...
    const liveDest = carZoomDest(null, fz, 0, lo, CAR_ZOOM_MAX);
    const from = carZoomRelease(zc, now, prevDest, carZoomRest(now, zc.pushAt, camZoomRef.current, carLiveZoomRef.current, prevDest), liveDest);
  ...
  const carCamJob = useRef((): boolean => {
    const now = Date.now();
    zoomLog(now, null);
    const lt = camLatTick(camRepairRef.current.lat, now);   // a waiting `cam-lat` row, once its gap allows (round 11)
    if (lt) { try { logEvent(lt); } catch {} }
    ...
    // lost: ONE corrective write is owed that restores what was written — always an instant push, never a fly (round 11)
    // — in episodes started by each owner intent, time-capped; see src/camRepair.ts. A pinch or any owner animation
    ...
    const rr = camRepairRow(rv);
    if (rr) { try { logEvent(rr); } catch {} }
  ...
  const getCam = useRef(() => {
    ...
    // target: it seeds the lockstep with the written pose (the CamWriteSeed path — zoom, pitch and, unless north-up
    // overrides it, the heading lag land exactly there), and this push writes it instantly. ROUND 11: the loop NEVER
    // flies — its 280 ms fly flew to the car's heading, not the written one, and every owner intent inside it re-aimed it.
    if (camRepairRef.current.pending) {
      camRepairRef.current.pending = null;
      const rw = camWantRef.current;
      if (rw) zoomSnapRef.current = { zoom: rw.zoom, pitch: rw.pitch ?? undefined, heading: typeof camHdgOverrideRef.current === 'number' ? undefined : rw.heading ?? undefined };
    }
    ...
    if (edge.fly) { returnFlyRef.current = -1; zoomChRef.current.ease = null; }
    // The overview ended (a press, recenter or compass) while crewFit's own native easeTo is STILL RUNNING: the snap's
  ...
  const applyZoomEased = (delta: number, src: string) => {
    const now = Date.now();
    // A new owner intent: the closed loop's fresh episode (src/camRepair.ts); a try it supersedes resolves in one row.
    const sr = camRepairEpisode(camRepairRef.current, now);
    if (sr) { try { logEvent(sr); } catch {} }
    const { followZoom: fz, previewMulti: pv } = camInputsRef.current;
    const lo = pv ? CAR_PREVIEW_ZOOM_MIN : CAR_ZOOM_MIN;
    const zc = zoomChRef.current;
    const via = now - zc.pushAt < CAR_ZOOM_MOVING_PUSH_MS ? 'push' : 'park';
    const prevDest = carZoomDest(manualZoomRef.current, fz, userZoomRef.current, lo, CAR_ZOOM_MAX);
    // A Crew overview (on, or lapsed with its way home still owed) or a return fly is moving the map natively: the
    // lockstep's camZoomRef is then the overview's TARGET or a stale chase zoom, so the ease starts from the VISIBLE
    ...
    if (flying) {
      // DURING THE RETURN FLY the fly carries the new framing: pushCam (the one owner) re-aims it from where the camera
  ...
  const takeOverNativeCam = (now: number, kind: 'gesture' | 'system' = 'gesture'): boolean => {
    const sr = camRepairEpisode(camRepairRef.current, now);   // a new owner intent: a fresh episode (src/camRepair.ts)…
    if (sr) { try { logEvent(sr); } catch {} }                 // …and a try it superseded resolves in one row
    ...
    return subscribeCarGesture((g: CarGesture) => {
      switch (g.kind) {
        ...
        case 'zoomBegin':
          ...
          // A pinch takes over from a +/- framing or an ease: it starts from the zoom pushCam is APPLYING now (mid-ease,
          // mid-release — not the ease's destination, not the bare follow zoom), carried as the pinch's bias, and ends
        ...
        case 'crewFit': {
          ...
          try {
            ...
            manualZoomRef.current = null;
            zoomChRef.current.ease = null;
            // camZoomRef is about to hold the fit TARGET (below) while the map animates there: the lockstep's last push
            // is no longer what is on screen, so no press may start from it (carZoomRest falls to the live zoom).
            zoomChRef.current.pushAt = 0;
            ...
            const easeAt = Date.now();
            crewEaseUntilRef.current = easeAt + CREW_FIT_EASE_MS;   // the native easeTo below runs until then
            const sr = camRepairEpisode(camRepairRef.current, easeAt);   // a new owner intent: the closed loop's fresh episode
            if (sr) { try { logEvent(sr); } catch {} }                    // (a try it superseded resolves in one row)
            ...
            cameraRef.current?.setCamera({
              ...
              pitch: 0, heading: 0, animationDuration: CREW_FIT_EASE_MS, animationMode: 'easeTo',
            });
            const lr = camLatCall(camRepairRef.current.lat, 'ease', easeAt, CREW_FIT_EASE_MS);   // `cam-lat` (round 11)
            if (lr) { try { logEvent(lr); } catch {} }
          } catch {}
          break;
        }
  ...
  reportDraw(
    ...
    !!s.navigating,
    // On this surface `lat`/`lng` ARE the raw fix (the car store never pins), so gps
  ...
  return (
    <MapView
      ...
      onCameraChanged={(state: any) => {
        noteCamObserved(state);   // the closed loop's eyes (src/camRepair.ts) — before anything can return
      ...
      onMapIdle={(state: any) => { noteCamObserved(state, true); selfRefreshRef.current?.(); noteMapIdle('car', true); }}
      onLayout={(e: any) => {
    ...
    >
      ...
      {hasFix && (
        <SelfCarModel
          ...
          camJob={carCamJob}
          camPoseOutRef={camWantRef}
          camCallOut={noteCamCall}
          cameraRef={cameraRef}
          getCam={getCam}
