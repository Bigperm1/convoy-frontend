...
- `node --experimental-strip-types tools/sim-qc/park_rearm_test.mts` — 🔒 privacy (Jeff, 2026-09-25: "My icon moved on the map"): the REAL `src/locationPrivacy.ts` after a witnessed park stays pinned through his 09-25 and 08-29 walks (recorded, densified, 41 s stuck on the multipath point, 14–31 s of 16 km/h readings first), two-outlier and ping-pong attacks and a car-park loop + outlier, and re-arms a real drive-away under `src/parkRearm.ts` v4 (pull-away ≤ 35 s, highway ≤ 30 s, red light ≤ 20 s after it, stop-sign grids ≤ 60 s, stop-and-go ≤ 75 s); slow congestion stays pinned 30 min and goes live ≤ 60 s after the jam clears (Q); the fifth review's walkers at 8 / 12 m noise, 1 Hz and Lite cadence, walk-then-sit and a moving-walkway concourse stay pinned (WK); the real-Δt claim (V5); a late storage read cannot be raced (Y); every witness starts the proof over; the witness drops the drive's latch (W/H2/S2); the head-unit sources (K); pinned rule values (V1); without a witnessed park every output is identical to the pre-fix module (U1). the review's canyon walker model (Z). Negative controls: N / W0 / H2a / K0 on the pre-fix module, Y0 / X0 / X0b / Z0 on round 3 (732c8a6e), Q0 / WK0 on round 4 (8a6fa0aa), V5c on round 4's claim, WK6-0 on the rule without its 10 s baseline check.
